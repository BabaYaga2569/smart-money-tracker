...
export class RecurringBillManager {
  ...
  static getBillsDueBefore(bills, beforeDate) {
    ...
    return bills.filter((bill) => {
      ...
      const dueDate =
        typeof dueDateValue === 'string' ? parseLocalDate(dueDateValue) : dueDateValue;
  ...
  static findBillPaidByTransaction(transactionId, bills) {
    ...
    return bills.find((bill) => {
      ...
      return paymentHistory.some((payment) => payment.transactionId === transactionId);
    });
  }
  ...
  static generateBillsFromTemplate(recurringTemplate, monthsAhead = 3, generateBillId) {
    ...
    const hasCustomRecurrence =
      activeMonths && Array.isArray(activeMonths) && activeMonths.length > 0;
    ...
    for (let i = 0; i < monthsAhead; i++) {
      const nextDueDate = this.getNextDueDate(
        currentBill,
        i === 0 ? today : new Date(currentBill.dueDate)
      );
  ...
  static syncBillsWithTemplate(updatedTemplate, existingBills, monthsAhead = 3, generateBillId) {
    ...
    const billsFromTemplate = existingBills.filter(
      (bill) => bill.recurringTemplateId === templateId
    );
    const otherBills = existingBills.filter((bill) => bill.recurringTemplateId !== templateId);

    const desiredBills = this.generateBillsFromTemplate(
      updatedTemplate,
      monthsAhead,
      generateBillId
    );
