...
export class RecurringManager {
  ...
  static getNextOccurrence(item, currentDate = new Date()) {
    ...
    const lastOccurrence = parseLocalDate(
      item.nextOccurrence || item.startDate || item.lastOccurrence
    );
    let nextOccurrence;
  ...
  static calculateNextOccurrenceAfterPayment(currentOccurrence, frequency) {
    ...
    if (!currentDate) {
      console.error(
        'calculateNextOccurrenceAfterPayment: Invalid currentOccurrence',
        currentOccurrence
      );
      return new Date();
    }
  ...
  static calculateNameSimilarity(name1, name2) {
    ...
    if (clean1.includes(clean2) || clean2.includes(clean1)) {
      return Math.max(
        80,
        (100 * Math.min(clean1.length, clean2.length)) / Math.max(clean1.length, clean2.length)
      );
    }
